...
3. **Dashboard localization** — 🟡 **In progress:** nav all-10 (#211) + **Overview page page-level, Hindi-full (#214)**. Pattern established: wrap page strings in `t()` (English-as-key) + add Hindi (flagship), regional falls back to English gracefully. **Baaki:** remaining pages (Agents/Calls/Analytics/Settings…) — `t()`-wrap + Hindi, then regional catalogs (catalog-only) with professional review before marketing non-Hindi.
...
6. Chhota (bacha hua): `packages/ui` + `apps/mobile` me zero tests; `provider-router/src/index.ts:~128` stale comment cleanup; `PROJECT-FEATURES-EXPLAINED.docx` regen (lxml sandbox me atka tha; `.md` current hai).
