...
3. **Dashboard localization** — 🟡 **In progress:** nav all-10 (#211) + page-level Hindi-full for **Overview (#214)** + **Agents & Calls (#215)** + **Leads (#218)**. Pattern: wrap page strings in `t()` (English-as-key) + add Hindi (flagship); regional falls back to English gracefully. **Baaki:** remaining pages (Analytics/Settings/Campaigns/Desk…) — same `t()`-wrap + Hindi, then regional catalogs (catalog-only) with professional review before marketing non-Hindi. (Enum DATA values — call direction/channel, lead status/stage — abhi English; alag mapping concern.)
