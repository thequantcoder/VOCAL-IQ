...
3. **Dashboard localization** — 🟡 **In progress:** nav all-10 (#211) + page-level Hindi-full for **Overview (#214)** + **Agents & Calls (#215)** + **Leads (#218)** + **Analytics (#219)** + **Campaigns (#220)**. Pattern: wrap page strings in `t()` (English-as-key) + add Hindi (flagship); regional falls back to English gracefully. **Baaki:** remaining pages (Settings/Desk/Appointments…) — same `t()`-wrap + Hindi, then regional catalogs (catalog-only) with professional review before marketing non-Hindi. **Gotcha:** English-as-key can't hold two senses of one word — a repeated English label (e.g. "Run" nav-section vs. button) must reuse ONE Hindi key (biome flags the duplicate; scan the `hi` block). Enum DATA values (call direction/channel, lead status/stage, campaign status) stay English — alag mapping concern.
