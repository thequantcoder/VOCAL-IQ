...
3. **Dashboard localization** — 🟡 **In progress:** nav all-10 (#211) + page-level Hindi-full for Overview (#214), Agents & Calls (#215), Leads (#218), Analytics (#219), Campaigns (#220), Settings›Appearance (#221), **Desk + Appointments + Squads + Voices (#222)**. Pattern: wrap page strings in `t()` (English-as-key) + add Hindi (flagship); regional falls back to English gracefully. **Baaki:** remaining pages (Phone numbers/Messaging/Callbacks/Payments/Integrations…) — same `t()`-wrap + Hindi, then regional catalogs (catalog-only) with professional review before marketing non-Hindi. **Gotchas:** (a) English-as-key can't hold two senses of one word — a repeated English label (e.g. "Run" nav-section vs. button) must reuse ONE Hindi key (biome flags the duplicate; scan the `hi` block); (b) enum DATA values (call direction/channel, lead status/stage, campaign status, theme control chips) stay English — alag mapping concern; (c) flaky billing.service deadlock (40P01) trips node CI ~1-in-2 — rerun the failed job, mera web change us se unrelated hai.
