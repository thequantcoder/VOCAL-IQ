...
3. **Dashboard localization** — 🟡 **Increment 1 done (#211):** poori sidebar nav (~60 labels) 10 India languages me (hi/bn/ta/te/mr/gu/kn/ml/pa/or as UI locales; English-as-key + per-key fallback). **Baaki:** page-level strings (headers/buttons/states, 119 files) — ab established `t()` pattern se per-page incremental; non-Hindi catalogs ka professional review before marketing.
