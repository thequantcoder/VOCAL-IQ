...
### FOLLOWUP-2 — Analytics trend tiles + PDF export — ✅ DONE
Two analytics polish pieces (CSV export + daily charts already existed in the base build):
- **Weekly trend tiles** — pure `windowedTrend(points, windowDays)` (shared) computes last-N-days sum + delta vs the prior N days (null when no prior window / prior is zero). The analytics page renders "Calls · last 7d" + "Cost · last 7d" tiles with the ▲/▼ delta, derived client-side from the already-fetched `callsByDay`/`costByDay` (no new query).
- **PDF export** — a **dependency-free** `buildSimplePdf({title, lines})` (shared) emits valid PDF 1.4 bytes (correct xref, Helvetica, PDF-escaped, non-ASCII stripped, one-page-truncated) — no bundle/dep cost. New route `GET /analytics/historical.pdf` reuses `AnalyticsService.historical` to build a summary (calls/minutes/success/cost/outcomes) and streams it `application/pdf`; a "PDF" button on the analytics page does an authenticated fetch → blob download (mirrors the CSV pattern).
- **Tests** — shared `analytics-trend.test` (delta, no-prior, zero-prior, negative) + `pdf.test` (well-formed header/xref/trailer/EOF, escaping, truncation, non-ASCII). typecheck/lint 12/12; shared **735/735**, full api **506/506**.

### FOLLOWUP-3 — Unified notification matrix — ✅ DONE
A per-tenant **event×channel matrix** that MASTER-gates the domain-event fan-out (layered on top of each channel's own config).
- **Scope call:** the domain-event emitter dispatches to **webhook + Slack** (email/WhatsApp are campaign/conversation channels, not domain-event notifiers — they'd need a "notify-destination" model, tracked separately). So the matrix covers webhook + Slack; the model (`channel: string`) is extensible for more later.
- **shared** (`notification-prefs.ts`): `NOTIFY_EVENTS` (5), `NOTIFY_CHANNELS` (webhook/slack), `CHANNEL_EVENTS` (which events each channel can deliver), `prefKey`, `notificationPrefsSchema` (sparse `record(string, boolean)`), and pure `isNotificationEnabled(prefs, event, channel)` — default ON, explicit-false-off (fail-open).
- **api**: `NotificationPrefsService` stores overrides in the tenant `settings` JSON (RLS-scoped, Slack-config pattern; never clobbers other keys). Routes `GET/PUT /notifications/preferences` (GET any member, PUT config-writers). The composition **`emitDomainEvent` now checks the matrix** before dispatching to webhook / Slack (fail-open if the prefs read fails).
- **web**: a Settings → **Notifications** page with the event×channel grid (checkboxes, default-on, unsupported cells show "—"), linked in the sidebar.
...
**Follow-up (2026-07-19):** extended the settings-race fix to **`compliance`** (writes `retention`) and **`residency`** (writes `residency`) — both moved to dedicated tenants (`…060a0003` / `…061a0003`), same proven pattern. Compliance keeps its global-suppression phone cleanup; the dedicated-tenant delete cascades agent/call/transcript/consent/settings. **Only `whitelabel` (Cloudflare custom-domain deps) + `reseller-dashboard` (reseller-hierarchy deps) remain deferred** — heavier seed dependencies, want a live DB to verify; after these fixes each is the sole writer of its shared settings row. api typecheck + biome green; CI validates the DB suites.

**Closure (2026-07-20):** the last two deferred suites — **`whitelabel`** and **`reseller-dashboard`** — are now moved off the shared seed tenants, **verified against a live local Postgres+RLS** (Docker brought up for the pass the earlier note wanted).
- **`whitelabel.service.test`** → dedicated customer `…052a0003` + reseller `…052a0002` (both children of PLATFORM, created in `beforeAll`, dropped in `afterAll`). The suite writes `settings.domain` on provision; it no longer read-modify-writes or **full-wipes** the shared `…0003.settings` (the single most destructive teardown — a `settings: {}` reset). Belt-and-suspenders: `beforeAll` frees the two claimed hostnames (`customDomain` is globally unique) from any stale owner so provisioning can't false-conflict. Branding/`customDomain` live in their own columns (not the shared `settings` JSON), so they were never the cross-suite hazard — only the `settings` wipe was.
- **`reseller-dashboard.service.test`** → dedicated reseller `…054a0003` + client `…054a0004` (alongside the existing rival `…054a0001/2`); the R1→C1 margin roll-up + the markup round-trip (`settings.markupBps`) now run entirely on the dedicated reseller. No more `settings: {}` reset of the shared reseller `…0002`. `is_in_subtree` walks `parentTenantId` live (no closure table), so the freshly-created child resolves its name under RLS — confirmed by the green overview join.

**Verification (live DB).** api typecheck ✅ · biome clean on both files ✅ · both suites green in isolation (8 tests) ✅ · **all 7 settings-touching suites run together in parallel** (`whitelabel` + `reseller-dashboard` + `disclosure` + `slack` + `compliance` + `residency` + `vault` = 34 tests) **all green** — the previously-flaky `disclosure` included ✅ · post-run DB inspection: shared `…0002`/`…0003` settings still `{}` (never wiped), zero leaked dedicated test tenants (afterAll cleanup verified). **The shared-tenant `settings` cross-suite race is now fully closed — every settings-writer owns its own tenant.**

## Self-Audit — settings-race closure (A–K)
A. Correctness (focus): ✅ — both suites assert identical behaviour on dedicated tenants; the reseller name-join resolves the fresh child via live `is_in_subtree`; the domain-uniqueness reject still fires across two dedicated tenants (checked via `db.admin`, RLS-bypass).
B. Isolation (focus): ✅ — RLS assertions preserved (child-never-sees-parent branding; rival margin never leaks into the reseller overview); the suites now touch ONLY tenants they own.
C. Security: ✅ — no secrets; Cloudflare stays faked; no new surface.
D. Cost: n/a — test-only; no calling/provider path.
E. Errors/obs: ✅ — teardown is delete-by-id (cascades); beforeAll is idempotent (upsert + margin `deleteMany` + hostname free) so reruns after a crash are clean.
F. Performance: n/a — test-only; dedicated-tenant setup is a handful of upserts.
G. Error handling: ✅ — `beforeAll` defensively frees stale `customDomain` so a leftover row can't false-conflict; `update: { settings: {} }` on the reseller upsert guarantees the default-markup (0) assertion even after a crashed prior run.
H. UI/AA: n/a — test-only.
I. Regressions (focus): ✅ — additive/isolating; no product code changed; api typecheck green; the 34-test parallel run (incl. every prior settings-writer) is green; the shared seed rows are provably untouched.
J. Quality/docs: ✅ — both files carry a header explaining WHY they run on dedicated tenants (the race); this log closes the deferral opened 2026-07-19.
K. Build/CI: ✅ — typecheck + biome green; the DB suites verified locally (Docker up) — the exact live-DB pass the deferral asked for; CI runs them too.
...
## Dashboard localization — page-level, increment 1: the Overview (landing) page

**Follow-up to #211** (which localized the nav across all 10 India languages). This starts **page-level** l10n on the highest-traffic surface — the dashboard Overview (landing) page every user sees first — establishing the concrete per-page template for the rest.

- **`app/dashboard/page.tsx`**: every visible string now renders via `useI18n().t()` (English-as-key) — the greeting (`Good morning/afternoon/evening`), the hero heading + CTAs (`Create an agent`, `Place a test call`), the 4 KPI labels (`Agents`/`Calls (recent)`/`Spend (recent)`/`Success rate`), `Recent activity` / `View all`, the empty-state line, and the `NextStepCard` (its 3 journey titles + hints, `What to do next`, `View analytics`, `Recent momentum`). No layout/logic change — a pure copy-externalization. `greeting()` stays a pure function returning the English key; the call site wraps it (`t(greeting())`).
- **`lib/i18n/catalogs.ts`**: the full Overview string set translated into **Hindi** (the flagship India language). Header comment documents the rollout model: **nav = all 10 locales; page copy = lead-language-first (Hindi now), others fall back to the English source per key until a professional pass** — graceful by the `translate` fallback (`catalog → en → key`), so a Tamil user sees localized nav + English page copy, never a missing string.
...
## GME-11 — rich RCS engine core + SMS fallback cascade — 2026-08-10 — 🧠 OPUS — ✅ DONE

Phase C (rich RCS) begins. The SMS-carrier phase (GME-05→10, 16 carriers) is done; GME-11 is the **pure, keys-none engine** for rich content + capability negotiation + graceful fallback. Concrete RBM/CPaaS adapters + the service send-path wiring land in GME-12.

- **`packages/shared/src/rcs.ts`** (pure, web-safe): `RichMessage` discriminated union — `text | card | carousel | media` + suggestion chips (`reply`/`action`), Zod-validated, mirroring Google RBM's AgentContentMessage (card ≤4 suggestions + needs title/desc/media; carousel 2–10 cards; chip label ≤25; text ≤3072; media SHORT/MEDIUM/TALL). `richMessageToText()` projects any rich message to the plain-text variant for the SMS/WhatsApp fallback leg (title/description/media-url/suggestion labels; carousel cards numbered `(i/n)`). `RcsProvider` interface (`capabilityCheck`, `sendRich`, optional `typingIndicator`/`readReceipt`). **Cascade engine:** `planCascade(rcsCapable, policy)` → ordered channel chain (RCS-first when capable + preferred, else fallbacks) and `runCascade(chain, attempt)` — tries each channel until one succeeds, records every `CascadeAttempt` + the `fallbackFrom`/`fallbackTo` provenance. `RcsCapabilityCache` — msisdn→capability TTL cache (6h default, injectable clock), mirrors the SMS router's in-memory health map.
- **DB:** `Message.richPayload Json?` (the RichMessage) + `fallbackFrom`/`fallbackTo` (which channel we preferred vs actually used). Additive nullable columns — migration `20260810160000`. Cost already differs by resolved channel via `messageCostUsd(channel, text)` (RCS 0.007 vs SMS per-segment), so a fallback re-prices automatically.
- No new enum values (RCS channel already exists); no exhaustive-map churn.

**Tests:** `packages/shared/src/rcs.test.ts` — rich schema validation (each kind + defaults; rejects <2-card carousel / empty card / over-long chip / unknown kind); `richMessageToText` per kind; `planCascade` (capable/​non-capable/​preferRcs-off/​multi-fallback); `runCascade` (first-success no-fallback, RCS→SMS fallback provenance, all-fail null, throw-as-failed-attempt); `RcsCapabilityCache` TTL expiry; cost-differs-by-channel. Next: **GME-12** (RCS providers — Google RBM + a CPaaS RCS adapter + wire the cascade into the send path).

## GME-12a — Google RBM provider (JWT auth + rich mapping + capability) — 2026-08-10 — 🧠 OPUS — ✅ DONE

First concrete `RcsProvider` (GME-11 seam). RBM is the native Google RCS channel — rich cards/carousels/suggestions/media. Split GME-12 into two green PRs: **12a = the RBM provider** (this), **12b = wire the cascade into the service + a CPaaS RCS adapter + rich DLR**.

- **`apps/api/src/messaging/rbm.ts`** — `RbmRcsProvider implements RcsProvider`. Auth = a Google **service account**: `GoogleServiceAccountAuth` RS256-signs a JWT with `node:crypto` (`createSign('RSA-SHA256')`, base64url, `\n`-unescape for env-copied PEMs), exchanges it at `oauth2.googleapis.com/token` for a bearer token, and **caches** it until ~60s before expiry — no googleapis SDK. `capabilityCheck` = GET `/v1/phones/{e164}/capabilities?agentId` (200→capable, 404→not, error→not so the cascade falls back). `sendRich` = POST `/v1/phones/{e164}/agentMessages?messageId={uuid}&agentId` with `{contentMessage}`; the client-generated messageId is the correlation/provider id. `richMessageToRbm()` (pure) maps a `RichMessage`→RBM `contentMessage` (text / `richCard.standaloneCard` / `richCard.carouselCard` / `contentInfo`), suggestions→`{reply}`/`{action:{openUrlAction|dialAction}}` (postbackData defaults to the label). A body-less-GET-capable `RbmHttp` transport (the SMS `HttpClient` mandates a body). `buildRbmProvider(env)` gated on `GOOGLE_RBM_AGENT_ID/CLIENT_EMAIL/PRIVATE_KEY`.
- `.env.example`: `GOOGLE_RBM_*`. No Prisma/enum changes (RCS channel + `Provider.RCS` already exist; billing maps in 12b).

**Tests:** `apps/api/src/messaging/rbm.test.ts` — `richMessageToRbm` per kind (+ reply/action suggestions); `GoogleServiceAccountAuth` mints a **real RSA-verifiable** RS256 JWT (generates a keypair, verifies the assertion signature with the public key + asserts claims) and caches (2nd call = no 2nd exchange); `capabilityCheck` 200→true/404→false; `sendRich` url (messageId+agentId) + body + id; non-2xx → failed result. Full apps/api biome clean; mapping + JWT sign/verify verified out-of-band (7/7). Next: **GME-12b** (cascade wiring into `messaging.service` + CPaaS RCS + rich DLR).

## GME-12b — wire the rich RCS cascade into the send path — 2026-08-10 — 🧠 OPUS — ✅ DONE

Connects the GME-11 cascade + GME-12a RBM provider to the real send path — rich messaging now works end-to-end (gated).

- **`MessagingService.sendRich(tenantId, {to, richMessage, policy?, …})`** — `parseRichMessage` validates → `richMessageToText` computes the fallback variant → RCS **capability** is checked via the injected `RcsProvider` (cached per-msisdn in `RcsCapabilityCache`, gated on `buildRbmProvider(process.env)` → `GOOGLE_RBM_*`) → `planCascade(capable, policy)` gives the ordered chain. When RCS leads + the provider sends OK, the message is persisted + metered here (`Provider.RCS`, `persistRcs`). Otherwise (RCS not-capable, or attempted-and-failed) it falls back to the first fallback channel via the existing **`send()`** with the text variant — so opt-out, +91 DLT, provider-routing, and UsageRecord metering ALL still apply — stamping `fallbackFrom`/`fallbackTo` when a fallback happened.
- `SendInput` gained optional `richPayload`/`fallbackFrom`/`fallbackTo`; `send()` persists them (`richPayload as object`, the repo's Prisma-Json pattern). `MessagingService` ctor gained `rcsProvider`/`rcsCapCache` opts (default: RBM from env + a fresh cache). No schema change (GME-11 added the columns).
- **Gated:** with no `GOOGLE_RBM_*`, `buildRbmProvider` → undefined → every rich send cascades straight to SMS. The existing `send()` behaviour is unchanged (rich fields default undefined).

**Tests:** `messaging.service.test.ts` (real Postgres + RLS) — a capable recipient gets RCS (channel RCS, `richPayload` persisted, `providerMessageId` from RBM); a non-capable recipient gets the SMS **text variant** (card title+description flattened); an RCS delivery failure falls back to SMS with `fallbackFrom=RCS`/`fallbackTo=SMS`; no-provider cascades straight to SMS. Full apps/api biome clean; the cascade decision matrix + planCascade verified out-of-band (5/5). Next: **GME-12c** (a CPaaS RCS adapter + rich DLR + a `/messaging/send-rich` route), then **GME-13** (rich-message studio UI).

## GME-12c — `/messaging/send-rich` API route — 2026-08-10 — ⚡ SONNET — ✅ DONE

Makes the rich-RCS cascade reachable from the API (until now `sendRich` was service-only). `POST /messaging/send-rich` (auth + tenant + `CONFIG_WRITERS`, mirroring `/send`) validates the body with `sendRichSchema` = `{ to, richMessage: richMessageSchema, policy?: cascadePolicySchema, contactId?, callId?, campaignId? }` and calls `MessagingService.sendRich(tenantId, …)`. A thin wrapper over the GME-12b-tested service; messaging routes are test-less by repo convention (the service carries the logic tests). biome clean. Next: **GME-12d** (a CPaaS RCS adapter — Sinch/Twilio — behind the `RcsProvider` seam + rich DLR typing/read→status), then **GME-13** (rich-message studio UI).

## GME-14 — messaging consent model + in-call consent capture — 2026-08-10 — 🧠 OPUS — ✅ DONE

**Phase D (consent-driven automation) begins** — the user's headline vision (inbound call → consent → SMS/WhatsApp/RCS follow-up). GME-14 is the consent foundation.

- **Prisma:** `Contact` gained `smsConsent`/`whatsappConsent`/`rcsConsent` (bool, default false) + `messagingConsentBasis` + `messagingConsentAt` (additive, migration `20260810180000`), mirroring the Day-72 email-consent shape. Each consent change also appends a `ConsentRecord(channel, granted, basis, region, ts)` (the model already existed).
- **Shared `messaging-consent.ts`** (pure/web-safe): `MESSAGING_CONSENT_CHANNELS`/`_BASES` + `setMessagingConsentSchema` + **`detectConsentIntent(transcript)`** → `{granted, channels[]} | null`. Heuristic: detect channel keywords (whatsapp/rcs/text|sms|message) + a "send you the details" cue; a decision needs an affirmative/negative cue and a **negative overrides** an affirmative (a "no" is never consent); no topic / no decision → null. Deterministic (keys-none) — the LLM extractor is injected by the post-call caller with this as fallback.
- **api `MessagingConsentService`:** `setConsent` (updates the denormalised Contact flags for fast gate reads + appends a per-channel `ConsentRecord` audit — grants AND revokes) · `getConsent` · `hasConsent(channel, phone)` (the choke-point helper GME-15 calls) · `captureFromTranscript` (runs `detectConsentIntent`, or an injected pre-extracted intent, and records it as `in_call_consent`). Tenant-scoped via `withTenant` (RLS). `/messaging/consent` routes (`POST` set = config-writers, `GET /:phone`) wired into `composition.ts` + `main.ts` (mounted before the `/messaging` catch-all).

**Tests:** shared `messaging-consent.test.ts` — `detectConsentIntent` (yes+text→SMS, named channel, multi-channel, negative-declines, send-cue default SMS, no-topic→null, channel-but-no-decision→null) + schema defaults; api `messaging-consent.service.test.ts` (real Postgres) — grant flips the flag + writes ≥1 audit record per channel with basis/region, per-channel revoke leaves the other channel granted, transcript "yes text me" captures SMS, a no-decision transcript records nothing. Full shared + apps/api biome clean; `detectConsentIntent` verified out-of-band (8/8). Next: **GME-15** (unified `MessagingGuard` — consent · opt-out · DNC · quiet-hours · rate · DLT, the single send choke point).

## GME-15 — unified messaging send-gate (MessagingGuard) — 2026-08-10 — 🧠 OPUS — ✅ DONE

The single recipient-eligibility choke point every send routes through — no bypass (mirrors the OutboundService rule for calls).

- **api `MessagingGuard.check(tenantId, ctx)`** → `{allowed, reason?}`, short-circuiting on the FIRST failing gate (never a silent drop): **opt-out** (`MessagingOptOut`, per channel) → **DNC/suppression** (`Suppression` + `Contact.dnc`) → **consent** (GME-14 `hasConsent`, enforced only when `ctx.requireConsent`) → **quiet-hours** (only when `ctx.respectQuietHours`; the tenant window from `Tenant.settings.messagingQuietHours` else the TCPA 8am–9pm default, evaluated at the recipient's approximate local time). `gateReasonMessage()` maps each reason to a user-facing message.
- **shared `messaging-guard.ts`** (pure): `MessagingGateReason`/`MessagingGateResult`, `messagingQuietHoursSchema` (enabled default false + 8–21 window), `phoneUtcOffsetMinutes(phone)` (representative UTC offset by E.164 country, longest-prefix-first), `isQuietTime(now, offsetMinutes, {startHour,endHour})` (blocked outside the local window).
- **Wired in:** `MessagingService.send()` replaced its inline opt-out check with `guard.check(...)` — so **DNC/suppression is now enforced on the send path too** (it wasn't before); `sendRich()` gates the RCS leg (a guard-blocked RCS falls through to the fallback channel, whose `send()` re-checks — so DNC blocks all channels while an RCS-only opt-out still falls to SMS). Ctor gained `opts.guard` (default `new MessagingGuard(db)`). Rate-limit (abuse, still first in `send`) + India DLT (content) stay put — orthogonal to recipient-eligibility. **Consent + quiet-hours are opt-in per send**, so transactional sends are behaviour-preserved; the GME-16 follow-up path opts into both. Kept the public `isOptedOut` (other callers/tests).

**Tests:** `messaging-guard.service.test.ts` (real Postgres) — clean allow, opt-out, suppressed, `Contact.dnc`, consent (no_consent → granted-allows, + the transactional path allows without requireConsent), quiet-hours (22:00-local blocked / 10:00-local allowed / not-respected allowed). shared `messaging-guard.test.ts` — offset map + `isQuietTime` + schema defaults. Full shared + apps/api biome clean; quiet-hours + offset verified out-of-band (9/9). Existing opt-out test message preserved (`/opted out/`). Next: **GME-16** (call-end → automation/workflow dispatch — fires the consent-gated follow-up; the vision's payoff).

## GME-16 — call-end → consent-gated follow-up (THE VISION'S PAYOFF) — 2026-08-10 — 🧠 OPUS — ✅ DONE

The whole engine's reason to exist: an inbound AI call ends → a matching automation → a **consent-gated, quiet-hours-respecting** SMS/WhatsApp follow-up fires — all on-platform. The automation ENGINE + `send_message` action + `dispatch` already existed (Day 47); GME-16 closed the two gaps.

- **Auto-dispatch on call-end:** `OutboundService.recordDisposition` (the hook the voice service calls at call end) now fires an optional `onDisposition(tenantId, callId, disposition)` — fire-and-forget, mirroring the existing form-extraction `onCallEnded` hook. Composition wires it (late-bound via a holder, since automations is built after outbound) to **`AutomationsService.dispatchCallEnded()`**, which resolves the call's contact (`contact.phone` → the recipient, `contactId`, `agentId`) and `dispatch({event:'call_ended', callId, disposition, agentId, contactId, to})`. `AUTOMATION_EVENTS` already had `call_ended`.
- **Consent-gate the follow-up:** the `send_message` automation action (shared `automation.ts`) gained `requireConsent?`/`respectQuietHours?`; `SendInput` + `MessagingService.send()` thread them into the GME-15 `guard.check` (was hardcoded false); the executor (`automations/executors.ts`) passes `action.requireConsent`/`respectQuietHours`. So a follow-up automation opts into the consent + quiet-hours gates; transactional automations (flags off) are unchanged.

**Tests:** `automations.service.test.ts` — `dispatchCallEnded` resolves a real Call's contact into the dispatched `call_ended` event's `to`/`contactId`, + the action carries `requireConsent` through the schema; `messaging.service.test.ts` — `send({requireConsent:true})` refuses without consent (`/consent/i`) and sends transactionally without it. Full shared + apps/api biome clean; guarded against a possibly-null `call` deref (extract-to-const before the conditional spreads — a tsc-only catch). Next: **GME-16b** (workflow path: `workflows.dispatchEvent(call_ended)` + a `send_message` workflow action + builder UI + RCS/cascade in the follow-up), then **GME-17** (message campaigns).

## GME-17 — message campaigns — 2026-08-10 — ⚡ SONNET — ✅ DONE

Send one template/body to a recipient list, consent-gated by default, every recipient through the guard.

- **`MessageCampaignService.send(tenantId, input)`** — a thin orchestrator over `MessagingService.send()`: de-dupes the recipients, sends each with `requireConsent`/`respectQuietHours` (both **default true** — a campaign is marketing, not transactional), and tallies a per-recipient summary `{total, sent, skipped:[{to,reason}], failed}`. A guard refusal (opt-out/DNC/consent/quiet-hours) or a missing template variable is a `skipped` with its reason (a `ValidationError` → caught); a QUEUED send (no provider) is skipped too; anything else is `failed`. Every send still routes through the GME-15 `MessagingGuard` + is metered — no bypass. List capped at 500 (larger paced campaigns → the deferred durable queue).
- **shared `messageCampaignSchema`** (in `messaging.ts`): channel SMS/WHATSAPP, templateId|body (refine), variables, recipients (1–500), requireConsent/respectQuietHours (default true), campaignId. `POST /messaging/campaign` (config writers) wired into composition + main (before the `/messaging` catch-all). Blended call→message journeys are already delivered by GME-16.

**Tests:** `message-campaign.service.test.ts` (real Postgres) — a mixed list (consented / opted-out / no-consent + a duplicate) → total de-duped to 3, sent 1 (consented only), 2 skipped with `/opted out/` + `/consent/` reasons, 0 failed; the schema rejects a campaign with no template/body. Full shared + apps/api biome clean. Next: **GME-13** (rich-message studio UI in apps/web).

## GME-13 — rich-message studio UI — 2026-08-10 — ⚡ SONNET — ✅ DONE

A visual rich-RCS composer on the Messaging dashboard page.

- **`apps/web/app/dashboard/messaging/rich-message-builder.tsx`** — `RichMessageBuilder`: a **text / card / carousel** editor (title/description/media-url per card, add/remove carousel cards up to 10), up to **4 suggestion chips** (reply, or an action when an Open-URL is filled), a recipient input, and a **live preview** rendering the composed message as a phone-style card mockup (media thumbnail + chips). Sends via **`useSendRichMessage`** (`POST /messaging/send-rich` → the GME-11/12 cascade: RCS when reachable, else SMS/WhatsApp text variant), then shows the resolved channel + status. Fully localized via `useI18n().t()` (English-as-key + Hindi). Wired into `messaging/page.tsx` under the ad-hoc send panel.
- **`lib/api.ts`** — `RcsSuggestion`/`RcsCard`/`RichMessageInput` DTOs + the `useSendRichMessage` TanStack mutation (invalidates the message log).
- **Lint discipline (web build is CI-only):** editor lists key by **stable per-item ids** (a `useRef` counter + lazy `useState` init → hydration-safe, restarts at 0 on both SSR + client), preview lists by **content keys** — no array-index keys (biome `noArrayIndexKey` is an error here). `img` has a static alt; buttons carry `type`/`aria-label`; `send.data &&` narrows the mutation result (no reliance on `isSuccess` narrowing).

**Checks:** full apps/web + apps/api + shared biome clean; design tokens (`vq-accent`/`vq-danger`/`vq-success`/`vq-surface*`/`vq-border`/`vq-text-*`) verified against existing usage. The Next.js build is validated by CI. **The GME engine is now functionally complete** — SMS (16 carriers) + rich RCS + consent-driven call→follow-up automation + campaigns + this studio UI, all gated + guarded. Optional polish remains (GME-16b workflow path, GME-18 provider-config UI, GME-12d CPaaS RCS, durable async queue).
