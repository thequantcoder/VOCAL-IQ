...
- **Phase 2 — `refactor/api-express`:** NestJS→Express + Clerk→JWT. Split into two green slices so `apps/api` is never broken:
  ...
  - **Phase 2b (next):** the flip — 16 controllers→routers, `main.ts`→Express bootstrap, composition root (manual DI), strip `@Injectable`/`@Inject` from services + PrismaService lifecycle, delete `*.module.ts` + Clerk (`clerk.ts`/`user-sync.ts`/`webhook.ts`) + guards, add register/login routes, remove `@nestjs/*`+`@clerk/*` deps. One buildable unit; services + service-tests stay green.
