...
Deviation/deferral: the API stores + resolves the per-agent policy, and the voice loop honours `LoopConfig.emotion_policy`, but the **api→voice hand-off of the policy at call start rides on the pending Day-17+ compiled-agent-config channel** (same seam that still defaults `voice_id`/`system_prompt` in `apps/voice/app/calls/router.py`); until then the loop applies the neutral default. Logged here per CODE-PATTERNS discipline.

Verification: shared **503** tests, api **336** tests (incl. new emotion + isolation), voice **98** pytest + **pyright 0 errors** + **ruff clean**, full **typecheck 12/12**, **lint 12/12** (warnings only — pre-existing `req.ctx!`), **build 8/8**. TS↔Python numeric parity verified identical across all tone/expressiveness combos. Migration applied locally on PG 16. Adversarial multi-dimension review (appropriateness/parity/latency/isolation/UI) run before commit.
