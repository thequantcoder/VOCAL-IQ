...
The blocker was that the credential-decrypting send path lives in `apps/api` and isn't reachable from `apps/workers` (which has no vault/adapters). **GME-DQ-a ✅:** `apps/api/src/messaging/messaging-internal.ts` — `POST /internal/messaging/send`, `MESSAGING_INTERNAL_SECRET`-guarded (constant-time via the shared `checkInternalSecret`; 503 gated / 401 unauthorized), runs one `MessagingService.send()` through the full `MessagingGuard` + metering. `executeInternalSend` maps a guard/validation refusal to a **permanent `SKIPPED`** (200, so a worker never retries it) and lets a transient error propagate (5xx → retryable). This is the sanctioned worker→api execute seam. **GME-DQ-b (TODO):** the bulk-send BullMQ worker in `apps/workers` (drains per-recipient jobs → POSTs the endpoint, retry/backoff) + an api enqueue path (a `MessageCampaign`/`Recipient` table + a worker tick, mirroring `campaign-scheduler`, OR bullmq-in-api) so a campaign > 500 recipients enqueues instead of sending synchronously.
