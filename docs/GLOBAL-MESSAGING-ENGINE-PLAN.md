...
**GME-12c — CPaaS RCS + rich DLR (TODO):** one CPaaS RCS adapter (Sinch/Twilio) behind the same `RcsProvider` seam + rich DLR (typing/read)→status + a `/messaging/send-rich` route.
