...
**GME-12b — cascade wiring into the send path · ✅ DONE (2026-08-10):** `MessagingService.sendRich()` — validates the RichMessage, checks RCS capability (cached, gated on `buildRbmProvider(env)`), plans the cascade (`planCascade`), and delivers over RCS when capable + preferred (metered as `Provider.RCS`) — else falls back to the first fallback channel with `richMessageToText` via the existing `send()` (so opt-out/DLT/metering all still apply), stamping `richPayload`/`fallbackFrom`/`fallbackTo` on the Message. `SendInput` gained the rich fields; `send()` persists them.
**GME-12c — `/messaging/send-rich` API route · ✅ DONE (2026-08-10):** `POST /messaging/send-rich` (CONFIG_WRITERS) — validates `{to, richMessage, policy?, contactId?, callId?, campaignId?}` with `richMessageSchema` + `cascadePolicySchema` and calls `MessagingService.sendRich`. Makes the rich-RCS cascade reachable from the API (thin wrapper over the tested service, like `/send`).
**GME-12d — CPaaS RCS + rich DLR (TODO):** one CPaaS RCS adapter (Sinch/Twilio) behind the same `RcsProvider` seam + rich DLR (typing/read)→status.
**DoD met (12b):** a rich card to an RCS-capable number sends via RBM; a non-RCS number cascades to SMS with the text variant + provenance. **Tests:** `messaging.service.test.ts` — capable→RCS, not-capable→SMS text variant, RCS-fail→SMS+provenance, no-provider→SMS.

### GME-13 — Rich-message studio UI · ⚡ SONNET · keys: none · ✅ DONE (2026-08-10)
**Built (`apps/web`):** `RichMessageBuilder` on the Messaging page — compose a **text / card / carousel** rich message with title/description/media-url fields, up to 4 **suggestion chips** (reply, or action when an Open-URL is given), a recipient, and a **live preview** (phone-style card mockup with media + chips). Sends via the cascade (`useSendRichMessage` → `POST /messaging/send-rich` → RCS with SMS/WhatsApp fallback), showing the resolved channel + status. Fully localized (`useI18n().t()` English-as-key + Hindi). `lib/api.ts` gained `RichMessageInput`/`RcsCard`/`RcsSuggestion` + the `useSendRichMessage` mutation.
**DoD met:** build + preview + send a rich RCS message from the UI (gated — cascades to SMS until `GOOGLE_RBM_*`). Editor lists use stable per-item ids (hydration-safe), preview lists content keys (no array-index keys). _Template-approval-status sync + a saved-rich-template library are a minor follow-up (the page already has SMS/WhatsApp template CRUD)._
...
### GME-15 — Unified send-gate (consent · opt-out · DNC · quiet-hours) · 🧠 OPUS · keys: none · ✅ DONE (2026-08-10)
**Goal:** One guarded path every message must pass (no bypass — mirror the OutboundService rule for calls).
**Built:** `MessagingGuard.check(tenantId, ctx)` — the single recipient-eligibility choke point, short-circuiting on the first failing gate with a clear `MessagingGateReason`: **opt-out** (`MessagingOptOut`, per channel) → **DNC/suppression** (`Suppression` + `Contact.dnc`) → **consent** (GME-14 `hasConsent`, opt-in via `requireConsent`) → **quiet-hours** (opt-in via `respectQuietHours`; TCPA 8am–9pm recipient-local by default, per-tenant override in `Tenant.settings.messagingQuietHours`). Shared `messaging-guard.ts` holds the pure parts (reasons, quiet-hours schema, `phoneUtcOffsetMinutes` + `isQuietTime`). `MessagingService.send()` now routes through the guard (replacing its inline opt-out check — so DNC/suppression is now enforced too) and `sendRich()` gates the RCS leg (a guard-blocked RCS falls through to the fallback, whose `send()` re-checks). Rate-limit (abuse, already first in `send`) + India DLT (content) stay in the send path — orthogonal to recipient-eligibility. **Consent + quiet-hours are opt-in per send** so transactional sends are unchanged; the consent-driven follow-up (GME-16) opts into both.
**DoD met:** a consent-required send with no consent is refused (`no_consent`); opt-out/DNC/suppression/quiet-hours each refuse with a reason; `send()`/`sendRich()` both route through the guard. **Tests:** `messaging-guard.service.test.ts` (real Postgres) — clean allow + each gate (opt-out/suppressed/dnc/no_consent→granted/quiet-hours) independently, incl. opt-in semantics; shared `messaging-guard.test.ts` — offset map + `isQuietTime` window + schema defaults.

### GME-16 — Call-end → automation dispatch (consent-gated follow-up) · 🧠 OPUS · keys: none · ✅ DONE (2026-08-10)
**Goal:** Actually fire the follow-up when a call ends — the vision's payoff (inbound call → consent → SMS/WhatsApp follow-up).
**Built (automation path):** `OutboundService.recordDisposition` now fires an optional `onDisposition` hook (fire-and-forget, like the existing form-extraction hook) wired in composition to **`AutomationsService.dispatchCallEnded(tenantId, callId, disposition)`** — which resolves the call's contact (phone + id) + agent and `dispatch(call_ended, {callId, disposition, agentId, contactId, to})`. The `send_message` automation action gained `requireConsent`/`respectQuietHours` (shared schema); the executor threads them to `MessagingService.send()`, which passes them to the GME-15 `MessagingGuard`. So a call-ended automation sends a **consent-gated, quiet-hours-respecting** follow-up (SMS/WhatsApp), while transactional automations are unchanged (flags default off). Late-bound via a holder (automations is built after outbound in composition).
**DoD met:** end an inbound call → matching `call_ended` automation → gated follow-up sent (blocked without consent). **Tests:** `automations.service.test.ts` — `dispatchCallEnded` resolves the call contact into the event recipient + carries `requireConsent` through the schema; `messaging.service.test.ts` — `send({requireConsent:true})` refuses without consent, sends transactionally without.
**GME-16b — workflow path + builder UI (TODO):** `workflows.dispatchEvent(call_ended)` + a `send_message` WORKFLOW action (worker `workflow-execution` executor, channel/cascade + template through the guard) + the workflow-builder `send_message` node config (localized) + RCS/cascade in the follow-up.

### GME-17 — Message campaigns · ⚡ SONNET · keys: none · ✅ DONE (2026-08-10)
**Built:** `MessageCampaignService.send(tenantId, {channel, templateId|body, variables?, recipients[], requireConsent, respectQuietHours, campaignId?})` — a thin orchestrator over `MessagingService.send()` so EVERY recipient routes through the GME-15 `MessagingGuard` + is metered. **Consent-gated + quiet-hours-respecting by DEFAULT** (a campaign is marketing, not transactional); the list is de-duplicated + capped (500 — larger paced campaigns belong on the deferred durable queue). Returns a per-recipient summary `{total, sent, skipped:[{to,reason}], failed}` — a gate refusal (opt-out/DNC/consent/quiet-hours) is a `skipped` with its reason, never a silent drop. `POST /messaging/campaign` (config writers) + `messageCampaignSchema` (shared). Blended call→message journeys are already covered by GME-16 (call-end → consent-gated follow-up).
**DoD met:** a campaign to a mixed list sends only to consented recipients, skipping opt-out + no-consent with reasons. **Tests:** `message-campaign.service.test.ts` (real Postgres) — consented→sent, opted-out→skipped, no-consent→skipped, list de-duped; schema requires a template/body.
...
`scripts/i18n` hand-off kit. **Progress: Phase A ✅ (#249–#255) · GME-05 India SMS (#256) · GME-06 DLT (#257) · GME-07 global SMS (#258) · GME-08 global SMS wave 2 (#259, recovered in `8421e39`) · GME-09 global SMS wave 3 (#260) · GME-10 India SMS wave 2 (#261) · GME-11 rich RCS engine core + cascade (#262) · GME-12a Google RBM adapter (#263) · GME-12b ✅ cascade wired into `sendRich()`. 16 SMS carriers + rich RCS (RBM + cascade + SMS fallback + `/messaging/send-rich` route) · GME-14 consent + GME-15 send-gate + **GME-16 call-end → consent-gated follow-up (THE VISION'S PAYOFF) · GME-17 message campaigns · GME-13 rich-message studio UI · **durable-bulk foundation** (internal guarded-send endpoint `POST /internal/messaging/send`, the worker→api execute seam)**. Next (optional polish): the bulk-send worker + enqueue (drains a queue → the internal endpoint) to lift the 500-recipient sync cap; GME-16b (workflow path); GME-18 (provider config UI); GME-12d (CPaaS RCS + rich DLR). **The GME engine is functionally complete — SMS (16 carriers) + rich RCS + consent-driven call→follow-up automation + campaigns + studio UI, all gated + guarded.**

### Durable bulk send queue (was "deferred") — foundation done (2026-08-11)
The blocker was that the credential-decrypting send path lives in `apps/api` and isn't reachable from `apps/workers` (which has no vault/adapters). **GME-DQ-a ✅:** `apps/api/src/messaging/messaging-internal.ts` — `POST /internal/messaging/send`, `MESSAGING_INTERNAL_SECRET`-guarded (constant-time via the shared `checkInternalSecret`; 503 gated / 401 unauthorized), runs one `MessagingService.send()` through the full `MessagingGuard` + metering. `executeInternalSend` maps a guard/validation refusal to a **permanent `SKIPPED`** (200, so a worker never retries it) and lets a transient error propagate (5xx → retryable). This is the sanctioned worker→api execute seam. **GME-DQ-b ✅ (2026-08-11):** the durable enqueue + data model. New tenant-scoped tables `MessageBulkJob` (the send spec + `total` + `RUNNING`/`DONE`) + `MessageBulkRecipient` (one PENDING row per recipient — the worker's unit of work: `PENDING → SENT/SKIPPED/FAILED` + reason + attempts), both RLS-enabled (migration `20260811120000`). `MessageBulkService.enqueue(tenantId, input)` de-dupes + persists the job + a `createMany` of PENDING recipients (≤50k), returns `{jobId, total}`; `status(jobId)` groupBy-aggregates the recipient counts. `POST /messaging/campaign/bulk` + `GET /messaging/campaign/bulk/:id` (config writers). Shared `messageBulkCampaignSchema` (≤50k) + `MessageBulkRecipientStatus`. **GME-DQ-c ✅ (2026-08-11):** the bulk-send worker — `apps/workers/src/bulk-send.ts`. A repeatable 10s tick (`registerBulkSend`, mirroring `campaign-scheduler`) drains PENDING `MessageBulkRecipient` rows for each RUNNING job (batch cap 100/job/tick) → `createInternalSend` POSTs each to `POST /internal/messaging/send` (DQ-a) → `runBulkSendTick` maps the outcome: `SENT`→row SENT · `SKIPPED`/`QUEUED` (guard refusal / gated provider)→row SKIPPED (permanent) · transient (non-2xx/network)→stays PENDING (retried next tick) until `maxAttempts` (3) then FAILED. A job with no PENDING left → DONE. Pure runner + injected deps (`createDbBulkSendDeps`) → unit-tested without Redis/HTTP/PG. **Gated:** the worker is only registered when `API_INTERNAL_URL` + `MESSAGING_INTERNAL_SECRET` are set (else bulk stays enqueued). **This closes the durable bulk path end-to-end — the 500-recipient sync cap is lifted (async path handles up to 50k/job, paced + guarded + retried).**
