    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      <div className="flex flex-col gap-1">
        <h1 className="flex items-center gap-2 font-display font-semibold text-vq-text-hi text-xl">
          <Gauge size={20} /> {t('Voice latency')}
        </h1>
        <p className="text-sm text-vq-text-lo">
          {t('Per-stage p50 / p95 vs SLO over the last 24h. Lower is snappier.')}
        </p>
      </div>
      ...
      {summary.isLoading ? (
        ...
        <EmptyState
          title={t('No latency samples yet')}
          hint={t('Once calls run, per-turn latency will chart here.')}
        />
      ) : (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-base">
              <span>{t('{n} turns', { n: summary.data.count })}</span>
              <span
              ...
              >
                {summary.data.breached ? t('SLO breached') : t('within SLO')}
              </span>
            </CardTitle>
          </CardHeader>
  ...
  return (
    <div className="flex flex-col gap-1 py-3">
      <div className="flex items-center justify-between text-sm">
        <span className="text-vq-text-hi">{t(STAGE_LABEL[stat.stage] ?? stat.stage)}</span>
