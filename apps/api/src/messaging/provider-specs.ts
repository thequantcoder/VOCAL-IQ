...
export const MESSAGING_PROVIDER_SPECS: Record<string, MessagingProviderSpec> = {
  ...
  kaleyra: {
    id: 'kaleyra',
    label: 'Kaleyra SMS (India)',
    channel: 'SMS',
    fields: [
      { key: 'apiKey', label: 'API key', secret: true },
      { key: 'sid', label: 'Account SID', secret: false },
      { key: 'sender', label: 'DLT sender header', secret: false },
      { key: 'apiDomain', label: 'API domain (default api.in.kaleyra.io)', secret: false },
    ],
    env: {
      apiKey: 'KALEYRA_API_KEY',
      sid: 'KALEYRA_SID',
      sender: 'KALEYRA_SENDER',
      apiDomain: 'KALEYRA_API_DOMAIN',
    },
  },
  fast2sms: {
    id: 'fast2sms',
    label: 'Fast2SMS (India)',
    channel: 'SMS',
    fields: [
      { key: 'apiKey', label: 'API authorization key', secret: true },
      { key: 'senderId', label: 'DLT sender id (6 chars)', secret: false },
      { key: 'entityId', label: 'DLT entity (PE) id', secret: false },
    ],
    env: {
  ...
  textlocal: {
    id: 'textlocal',
    label: 'Textlocal (India)',
    channel: 'SMS',
    fields: [
      { key: 'apiKey', label: 'API key', secret: true },
      { key: 'sender', label: 'DLT sender header', secret: false },
    ],
    env: { apiKey: 'TEXTLOCAL_API_KEY', sender: 'TEXTLOCAL_SENDER' },
  },
  'route-mobile': {
    id: 'route-mobile',
    label: 'Route Mobile SMS (India)',
    channel: 'SMS',
    fields: [
      { key: 'username', label: 'HTTP account username', secret: false },
      { key: 'password', label: 'HTTP account password', secret: true },
      { key: 'source', label: 'DLT sender header', secret: false },
      { key: 'host', label: 'API host (default api.rmlconnect.net)', secret: false },
    ],
    env: {
      username: 'ROUTEMOBILE_USERNAME',
      password: 'ROUTEMOBILE_PASSWORD',
      source: 'ROUTEMOBILE_SOURCE',
      host: 'ROUTEMOBILE_HOST',
    },
  },
  'whatsapp-cloud': {
    id: 'whatsapp-cloud',
...
const PROVIDER_ENUM: Record<string, Provider> = {
  ...
  kaleyra: Provider.KALEYRA,
  fast2sms: Provider.FAST2SMS,
  textlocal: Provider.TEXTLOCAL,
  'route-mobile': Provider.ROUTE_MOBILE,
